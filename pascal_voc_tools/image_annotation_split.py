...
class SplitImageAnnotation():
    ...
    def match_jpg_xml(self):
        ...
        images_name = [
            os.path.basename(path).split('.')[0] for path in images_path
        ]
        xmls_name = [
            os.path.basename(path).split('.')[0] for path in xmls_path
        ]
    ...
    def split_dir(self):
        ...
        for index, (jpg_path,
                    xml_path) in enumerate(zip(images_path, xmls_path)):
            ...
            for i, (image, xml_annotations) in enumerate(zip(images, xml_annotations_list)):
                save_xml_path = os.path.join(
                    ...
                    os.path.basename(xml_path).replace(
                        '.xml', '_{:0>2d}.xml'.format(i)))
                save_jpg_path = os.path.join(
                    self.save_jpg_dir,
    ...
    def split_image_annotations(self,
                                jpg_path,
                                xml_path,
                                cover_thresh=0.2,
                                iou_thresh=1.0,
        ...
        split_bboxes = self.get_split_bboxes(width=image.shape[1],
        ...
        for txt_path in [train_path, val_path, test_path]:
            with open(txt_path) as file:
                ...
                for name in name_list:
                    new_path_list += glob.glob(
                        os.path.join(self.save_xml_dir,
                                     name.strip() + '*.xml'))
                new_name_list = [
                    os.path.basename(name).split('.')[0]
                    for name in new_path_list
                ]
                with open(os.path.join(save_path, os.path.basename(txt_path)),
                          'w') as f:
                    f.write('\n'.join(new_name_list))
...
def test():
    ...
    spliter = SplitImageAnnotation(jpg_dir, xml_dir, save_jpg_dir,
                                   save_xml_dir)
    spliter.split_dir()
